...
// Load EmailJS only after auth is confirmed — keeps it out of the auth flow
  ...
  try {
    ...
    if (!userDoc.exists) {
      await db.collection('users').doc(user.uid).set({
        name: user.email, email: user.email, role: 'dsa',
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      currentDSA = { uid: user.uid, name: user.email, email: user.email, role: 'dsa' };
    } else {
    ...
    loadStats();
    startPendingBadge();
    loadEmailJS(); // safe to load now — user is confirmed
  } catch(err) {
    currentDSA = { uid: user.uid, name: user.email, email: user.email, role: 'dsa' };
    document.getElementById('dsaName').textContent = user.email;
    loadRequests('pending');
    loadStats();
    startPendingBadge();
    loadEmailJS();
...
function confirmAction(docId, status) {
  ...
  if (status === 'approved') {
    ...
    msg.textContent        = "This will approve the student's exeat request. They will be notified by email.";
...
async function executeAction() {
  ...
  try {
    // 1. Update Firestore
    await db.collection('exeatRequests').doc(docId).update({
      status,
      dsaComment: comment,
...
function logout() {
  if (pendingListener) pendingListener();
  auth.signOut().then(() => window.location.href = 'login-dsa.html');
}
...
function showToast(msg, type='success') { const t=document.getElementById('toast'); t.textContent=msg; t.className=`toast ${type} show`; setTimeout(()=>t.classList.remove('show'),4000); }
